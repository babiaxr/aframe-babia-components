...
| legend_lookat                 | Element that the legend will follow in terms of rotation                                                                                                                                     | string                 | `[camera]`    |
| legend_scale                  | Scale for the legend                                                                                                                                                                        | number                 | 1             |
| legend_text                  | Template for the text shown in legends. Supports placeholders like `{name}`, `{height}`, `{fheight}`, `{color}`, `{fcolor}`, etc. Use `\n` to create new lines. | string                 | `{name}\n{fheight}: {height}\n{farea}: {area}\n{fwidth}: {width}\n{fdepth}: {depth}\n{fcolor}: {color}` |
| legend_text | Template for the text shown in legends. Supports dynamic placeholders to display field names and values. Use `\n` to insert new lines. <br><br>**Supported placeholders:**<br>• `{name}` – Name of the element.<br>• `{height}` – Value of the metric defined by the `height` field.<br>• `{area}` – Value of the metric defined by the `area` field.<br>• `{width}` – Value of the metric defined by the `width` field.<br>• `{depth}` – Value of the metric defined by the `depth` field.<br>• `{color}` – Value of the metric defined by the `color` field.<br>• `{fheight}` – Name of the field used for `height` (e.g. `loc`).<br>• `{farea}` – Name of the field used for `area`.<br>• `{fwidth}` – Name of the field used for `width`.<br>• `{fdepth}` – Name of the field used for `depth`.<br>• `{fcolor}` – Name of the field used for `color`.<br><br>**Tip:** When used on quarters (i.e. elements with children), only `{name}` is shown regardless of the template. | string | `{name}\n{fheight} (height): {height}\n{farea} (area): {area}\n{fwidth} (width): {width}\n{fdepth} (depth): {depth}\n{fcolor}: {color}` |
